...
func (b *NodeBuilderImpl) tryAddUndefinedToTypeNode(typeNode *ast.Node) *ast.Node {
	if typeNode == nil || pseudochecker.TypeNodeCouldReferToUndefined(typeNode) {
		return nil
	}
	clone := b.reuseTypeNode(typeNode)
	if ast.IsUnionTypeNode(clone) {
		types := slices.Clone(clone.AsUnionTypeNode().Types.Nodes)
		types = append(types, b.f.NewKeywordTypeNode(ast.KindUndefinedKeyword))
		return b.f.NewUnionTypeNode(b.f.NewNodeList(types))
	}
	return b.f.NewUnionTypeNode(b.f.NewNodeList([]*ast.Node{clone, b.f.NewKeywordTypeNode(ast.KindUndefinedKeyword)}))
}
...
func (b *NodeBuilderImpl) serializeTypeForDeclaration(declaration *ast.Declaration, t *Type, symbol *ast.Symbol, tryReuse bool) *ast.Node {
	...
	if addUndefinedForParameter {
		if typeNode := b.tryAddUndefinedToTypeNode(declaration.Type()); typeNode != nil {
			return typeNode
		}
